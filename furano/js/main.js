...
// ──────────────────────────────────────
// .hero__text: #restaurantより上で表示、以降で非表示
// ──────────────────────────────────────
;(function () {
  const heroText = document.querySelector('.hero__text')
  const restaurant = document.querySelector('#restaurant')
  if (!heroText || !restaurant) return

  ScrollTrigger.create({
    trigger: '#restaurant',
    start: 'top bottom',
    onEnter: () => gsap.set(heroText, { autoAlpha: 0 }),
    onLeaveBack: () => gsap.set(heroText, { autoAlpha: 1 }),
  })
})()
...
;(function () {
  ...
  let savedTop = null
const CONCEPT_TEXT_SCROLL_RATE = 0.35
      ...
      onEnter: () => {
        ...
        conceptTextScrollListener = () => {
          ...
          conceptTextRafId = requestAnimationFrame(() => {
            conceptTextRafId = null
            const dy = (window.scrollY - scrollYAtEnter) * CONCEPT_TEXT_SCROLL_RATE
...
function initMap() {
  const position = { lat: 43.41233555838843, lng: 142.41910270990738 }

  const map = new google.maps.Map(document.getElementById('furano-map'), {
    zoom: 14,
    center: position,
  ...
  map.setOptions({
    styles: [
      { featureType: 'all', elementType: 'geometry', stylers: [{ color: '#eeeeee' }] },
      ...
      { featureType: 'water', elementType: 'geometry', stylers: [{ color: '#949494' }] },
      { featureType: 'landscape', elementType: 'geometry', stylers: [{ color: '#BDBDBD' }] },
      { featureType: 'landscape.natural.landcover', elementType: 'geometry', stylers: [{ color: '#b5b5b5' }] },
      { featureType: 'landscape.natural.terrain', elementType: 'geometry', stylers: [{ color: '#949494' }] },
      { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#656565' }] },
      { featureType: 'road', elementType: 'geometry.stroke', stylers: [{ color: '#555555' }] },
      { featureType: 'poi', elementType: 'geometry', stylers: [{ color: '#d8d8d8' }] },
      { featureType: 'poi', elementType: 'labels', stylers: [{ visibility: 'off' }] },
      { featureType: 'transit', elementType: 'all', stylers: [{ visibility: 'off' }] },
      { featureType: 'administrative', elementType: 'geometry.stroke', stylers: [{ color: '#bbbbbb' }, { weight: 0.5 }] },
    ],
  })

  const pinSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="44" viewBox="0 0 32 44">
    <path d="M16 0C7.163 0 0 7.163 0 16c0 11.5 16 28 16 28S32 27.5 32 16C32 7.163 24.837 0 16 0z" fill="rgba(204,0,0,0.9)"/>
