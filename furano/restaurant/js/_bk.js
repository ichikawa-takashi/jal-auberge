...
;(function () {
  ...
  function createSp() {
    ...
    spSt = ScrollTrigger.create({
      trigger: messageText,
      start: () => {
        const tmTop = parseInt(getComputedStyle(titleMain).top, 10) || 0
        const tmBottom = tmTop + titleMain.offsetHeight
      ...
      onEnter: () => {
        const main = document.querySelector('.main')
        const mtTop = window.scrollY + messageText.getBoundingClientRect().top
        const tmH = titleMain.offsetHeight
        const relTop = mtTop - SP_GAP - tmH - main.offsetTop
